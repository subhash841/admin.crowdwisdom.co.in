...
class Poll_model extends CI_Model {
    ...
    function get_polls() {
        ...
        $this -> db -> group_by( "p.id" );
        $this -> db -> order_by( "p.is_approved ASC, p.id DESC" );
        $this -> db -> where( "p.is_active = '1' AND p.is_public = '1'" );
//                $this -> db -> limit( 10, $offset );
        return $this -> db -> get() -> result_array();
//                
//                $this -> db -> select( "p.*,pc.name as category,group_concat(pch.choice) as choices,right_choice" );
...
//                return $this -> db -> get() -> result_array();
    ...
    function get_poll_details( $poll_id ) {
        ...
        $this -> db -> from( "topic_association ta" );
        $this -> db -> where( "post_id = '$poll_id' AND ta.type = 'Poll'" );
    ...
    function private_topic_mail_ids($topic_ids){
        $this -> db -> select( "GROUP_CONCAT(DISTINCT email_ids) as email_ids,is_private" );
        $this -> db -> from( "topics" );
    ...
    function add_update_poll( $inputs ) {
        ...
        if ( isset( $inputs[ 'topics' ] ) ) {
            ...
            $res=$this->private_topic_mail_ids($topics);
            $email_ids=$res['email_ids'];
            if($res['is_private']=='1'){
                $is_public="2";
            }else{
        ...
        if ( $poll_id == 0 ) {
            ...
            $this->private_topic_poll_mail($email_ids,$pollid,$poll_title);
...
//                echo "inside if";exit;
                ...
                $this->private_topic_poll_mail($email_ids,$poll_id,$poll_title);
                /* Update Poll details */
    ...
    function private_topic_poll_mail($emails,$poll_id,$title){

        $message .= "Hello, <br /><br />";
         
        $message .= "New Question - $title.<br /><br />";
        $message .= "To participate: <br /> https://www.crowdwisdom.co.in/Predictions/details/" . $poll_id;
        $message .= "<br /><br />";
        $message .="Win Amazon Voucher for answering this,";
        $message .= "<br /><br />";
        $message .= "Regards,<br /><br />CrowdWisdom Team";
        send_email($emails, '', $title, $message, 'send_bcc');
    }
