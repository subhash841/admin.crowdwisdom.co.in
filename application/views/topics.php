                            ...
                            <table class="table table-responsive table-bordered table-striped table-hover dataTable js-basic-example">
                                ...
                                <tbody>
                                    ...
                                    foreach ( $topics as $key => $list ):
                                            ...
                                            $topic_type = $list[ 'is_private' ] == 1 ? "Private" : "Public";
            ...
            <form name="addUpdateTopic" id="form_validation" method="POST" enctype="multipart/form-data" autocomplete="off" novalidate="novalidate">
                <div class="modal-body">
                    ...
                    <div class="form-group form-float m-t-10">
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <input name="is_private" type="radio" id="radio_type_is_public" class="with-gap radio-col-red" required="" value="0" checked>
                                            <label for="radio_type_is_public">Public</label>
                                        </div>
                                        <div class="col-sm-3">
                                            <input name="is_private" type="radio" id="radio_type_is_private" class="with-gap radio-col-red" required="" value="1" >
                                            <label for="radio_type_is_private">Private</label>
                                        </div>
                                    </div>
                                </div>
                        <div class="form-group form-float" id='email_ids' style="display:none">
                                <div class="form-line">
                                <input type="text" class="form-control" name="emails" required>
                                    <label class="form-label">Email Id's</label>
                                </div>
                                <i class="small">Enter comma saperated email ids</i>
                            </div>
                        </div>
...
//        });
        ...
        $("#uploadBtn").on("change", function (e) {
            //console.log(this.files[0]); // do not remove this line

            var file = $(this)[0].files[0];
            //var img = URL.createObjectURL(file);
            $('.imagerror').html('');
            $(".imagerror").removeAttr("style");
            // console.log(file['type'])
            var imageData = new FormData();
            imageData.append('file', file);
            validFormats = ['image/jpeg', 'image/jpg', 'image/png'];
                if (validFormats.includes(file['type']) == false) {
                    $('.imagerror').html('*Only  jpg | jpeg | png image files are supported.');
                    $('.imagerror').css('color', 'red');
                    return;
                } else if (file['size'] > 1048576) {
                    $('.imagerror').html('Maximum file size is up to 1 MB.')
                    $('.imagerror').css('color', 'red');
                    return;
                }else{
                    ...
                    img.onload = function () {
                        ...
                        if (iw >= 200 && ih >= 200) {
                            ...
                            $('.imagerror').html('');
                            $(".imagerror").removeAttr("style");
                            /* $('.imagerror').html('*Only Upload File with dimension 100x100'); */
                        ajax_call_multipart(uploadUrl, "POST", imageData, function (result) {
                            ...
                            $('.imagerror').html('*Only Upload File with dimension 200x200');
                            $('.imagerror').css('color', 'red');
                            return;
                        }
                  };
