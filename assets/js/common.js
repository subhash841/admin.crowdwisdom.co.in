...
//    });
    ...
    function validateform(type) {
        ...
        if (type == "addUpdateBlog") {
            ...
            if ($(forname + ' textarea[name="meta_keywords"]').val() == "") {
                isValid = false;
            }
            if ($(forname + ' textarea[name="meta_description"]').val() == "") {
                isValid = false;
            }
...
$("#topicModal").on("show.bs.modal", function (event) {
    ...
    if (id == "0") {
        $('#defaultModalLabel').html('ADD');
        $(base + "[name='topic_name']").attr('required', 'required');
        $(".topic_cat").attr('required', 'required');
        $(base + "[name='topic_img']").attr('required', 'required');
        $(base + "[name='topic_icon']").attr('required', 'required');
        $(base + "[name='topicid']").val("0");
        $(base + "[name='topic_name']").val('');
        $(base + "[name='uploaded_filename']").val('');
        $(base + "[name='uploaded_icon_filename']").val('');
        $(base + ".uploaded-img-preview").html('');
        $(base + ".uploaded-icon-preview").html('');
        $(".topic_cat option").removeAttr("selected");
        $('.topic_cat').selectpicker('render');
        $(base + "[name='topic_trending']").prop('checked', false);
        ...
        console.log(data)
        $(".topic_cat option").removeAttr("selected");
        $(base + "[name='topic_name']").removeAttr("required");
        if (data.image != "") {
            $(base + "[name='topic_img']").removeAttr("required");
        } else {
            $(base + "[name='topic_img']").attr('required', 'required');
        }
        if (data.is_private == "0") {
            $(base +'input[id="radio_type_is_public"]:checked').val();
            
        } else {
            $(base +'input[id="radio_type_is_private"]:checked').val();
        }
        if (data.icon != "") {
            $(base + "[name='topic_icon']").removeAttr("required");
        }
        if (data.category != "") {
            $(".topic_cat").removeAttr("required");
        }
        var id = data.id;
        var topic = data.topic;
        var email_ids = data.email_ids;
        var category_id = data.category;
        var is_trending = data.is_trending;
        var is_private = data.is_private;
        $(base + "[name='topicid']").val(id);
        $(base + "[name='topic_name']").val(topic);
        $(base + "[name='emails']").val(email_ids);
        $(base + "[name='uploaded_filename']").val(data.image);
        $(base + "[name='uploaded_icon_filename']").val(data.icon);
        $(base + ".uploaded-img-preview").html('<div class="" style="height:90px; background:url(' + data.image + ') center center no-repeat;background-size:contain;"></div>');
        $(base + ".uploaded-icon-preview").html('<div class="" style="height:90px; background:url(' + data.icon + ') center center no-repeat;background-size:contain;"></div>');
        if (is_private == 1) {            
            $( base +"#radio_type_is_private").prop( "checked", true );
            $("#email_ids").show();
        }
        
    
        $('.topic_cat option[value=' + category_id + ']').attr('selected', 'selected');
        ...
        if (is_trending == 1) {
            $(base + "[name='topic_trending']").prop('checked', true);
        } else {
            $(base + "[name='topic_trending']").prop('checked', false);
        }
...
$('form[name="addUpdateTopic"]').on('submit', function (event) {
    event.preventDefault();
    var topic_id = $("#topicid").val();
    var param = $(this).serialize();
    //console.log(param);
    if (validateAllforms("addUpdateTopic", topic_id)) {
        ajax_call(base_url + 'Master/addUpdateTopic', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
                $('#topicModal [data-dismiss="modal"]').trigger('click');
                setTimeout(function () {
...
$(document).on('change', '.changeActiveTopic', function (e) {
    ...
    var param = {id: id, type: type, current: status};
    ajax_call(base_url + 'Master/changeActiveTopic', "POST", param, function (result) {
...
$(document).on('change', '.changestopsurvey', function (e) {
    ...
    if (type != "") {
        ...
        console.log(param);
        ajax_call(base_url + 'Survey/stop_active_survey', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
...
$(document).on('change', '.changeTrendingTopic', function (e) {
    ...
    var param = {id: id, type: type, current: status};
    ajax_call(base_url + 'Master/changeTrendingTopic', "POST", param, function (result) {
...
$("#adminModal").on("show.bs.modal", function (event) {
    ...
    if (id == "0") {
        $('#defaultModalLabel').html('ADD');
        $(base + "[name='adminid']").val("0");
        $(base + "[name='admin_email']").val('');
        //$(base + "[name='admin_password']").val('');
    } else {
        $('#defaultModalLabel').html('EDIT');
        var data = relatedTarget.attr("data-editjson");
        data = JSON.parse(data);
        ...
        $(base + "[name='adminid']").val(id);
        $(base + "[name='admin_email']").val(email);
        //$(base + "[name='admin_password']").val('');    
    }
});
...
$('form[name="addUpdateAdmin"]').on('submit', function (event) {
    event.preventDefault();
    var admin_id = $("#adminid").val();
    var admin_email = $("input[name='admin_email']").val();
    var admin_pas = $("input[name='admin_password']").val();
    var param = $(this).serialize();

    //alert(admin_id + ' - ' + admin_email + ' - ' + admin_pas);

    if (admin_id == 0 && (admin_email == "" || admin_pas == "")) {
        showToast("Please enter all fields", '0');
        return false;
    }

    if (admin_id != 0 && admin_email == "") {
        showToast("Please enter all fields", '0');
        return false;
    }

    //console.log(param);
    if (validateAllforms("addUpdateAdmin", admin_id)) {
        ajax_call(base_url + 'Admin/addUpdateAdmin', "POST", param, function (result) {
            console.log(result);
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
                $('#adminModal [data-dismiss="modal"]').trigger('click');
                setTimeout(function () {
                    window.location.assign('lists');
                }, 2000);
            } else {
...
$(document).on('change', '.changeActiveAdmin', function (e) {
    ...
    var param = {id: id, type: type, current: status};
    ajax_call(base_url + 'Admin/changeActiveAdmin', "POST", param, function (result) {
...
/*------------- Wall ------------------*/
$(document).on('change', '.changeactivewall', function (e) {
    ...
    if (type != "") {
        ...
        var param = {survey_id: wall_id, type: type, current: status};
        ajax_call(base_url + 'Wall/active_inactive_wall', "POST", param, function (result) {
            result = JSON.parse(result);
            if (result['status']) {
                showToast(result['message'], '1');
                setTimeout(function () {
...
/*-------------from the web ------------------*/
$(document).on('change', '.changefromtheweb', function (e) {
    ...
    if (type != "") {
        ...
        var param = {web_id: web_id, type: type, current: status};
        ajax_call(base_url + 'FromTheWeb/active_inactive_web', "POST", param, function (result) {
...
function validateAllforms(type, id) {
    var isValid = true;
    /* alert($(forname + ' input[name="topic_img"]').val());
    alert($(forname + ' input[name="topic_icon"]').val());
    alert($(forname + ' input[name="uploaded_filename"]').val() == ""); */
    if (type == "addUpdateTopic") {
        var forname = "form[name=" + type + "]";
        if ($(forname + ' input[name="topic_name"]').val() == "") {
            isValid = false;
        }
        if ($(forname + ' input[name="topic_img"]').val() == "" && $(forname + ' input[name="uploaded_filename"]').val() == "") {
            isValid = false;
        }
        if ($(forname + ' input[name="topic_icon"]').val() == "" && $(forname + ' input[name="uploaded_icon_filename"]').val() == "") {
            isValid = false;
        }
        if ($('[name="blog_id"]').val() == '0') {
            isValid = false;
        }
    }
